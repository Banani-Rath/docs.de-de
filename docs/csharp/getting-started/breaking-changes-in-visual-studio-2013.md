---
title: "Unterbrechende Visual C#-&#196;nderungen in Visual Studio 2013 | Microsoft Docs"
ms.custom: ""
ms.date: "11/17/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: ""
ms.suite: ""
ms.technology: 
  - "devlang-csharp"
ms.tgt_pltfrm: ""
ms.topic: "article"
dev_langs: 
  - "CSharp"
helpviewer_keywords: 
  - "Unterbrechende Änderungen [C#]"
ms.assetid: 399802e7-dd13-4bb6-9b41-8ab1446a322c
caps.latest.revision: 34
caps.handback.revision: 34
author: "BillWagner"
ms.author: "wiwagn"
manager: "wpickett"
---
# Unterbrechende Visual C#-&#196;nderungen in Visual Studio 2013
Keine Änderungen in Visual C\# in [!INCLUDE[vs_dev12](../../csharp/getting-started/includes/vs_dev12_md.md)] verhindern, dass eine Anwendung, die in Visual C\# in Visual Studio 2012 erstellt wurde, kompiliert wird oder dass das Laufzeitverhalten einer solchen Anwendung geändert wird.  
  
## Siehe auch  
 [Getting Started with Visual C\#](../../csharp/getting-started/getting-started-with-csharp.md)   
 [Lambda\-Ausdrücke](../../csharp/programming-guide/statements-expressions-operators/lambda-expressions.md)   
 [Benannte und optionale Argumente](../../csharp/programming-guide/classes-and-structs/named-and-optional-arguments.md)   
 [params](../../csharp/language-reference/keywords/params.md)   
 [dynamic](../../csharp/language-reference/keywords/dynamic.md)   
 [Wann wird der Schutz einer Fehlerbehebung in der Programmiersprache aufgehoben?](http://go.microsoft.com/fwlink/?LinkId=259542)